...
def weighted_average_cost_of_capital(stock, date=datetime.now(), lookback_period: timedelta = timedelta(days=0),
                                     ...
                                     beta_period='Monthly', holding_period: str = 'Yearly', benchmark: str = '^GSPC'):
    from_date = date - lookback_period - lookback_lookback_period
    to_date = date - lookback_period

    dict = {'Common Equity': (cost_of_equity_capm(stock=stock, from_date=from_date, to_date=to_date,
    ...
    capitals = [np.nan_to_num(v[1]) for k, v in dict.items()]
    weights = [part / sum(capitals) for part in capitals]
    costs = [np.nan_to_num(v[0]) for k, v in dict.items()]
