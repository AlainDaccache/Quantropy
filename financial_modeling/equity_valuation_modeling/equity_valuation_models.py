...
It is therefore more flexible, realistic (thus more complex) and practical, considering fluctuation of the business 
...
def dividend_discount_model_two_stage(current_dividend: float, discount_rate: float, growth_rate: float,
                                      long_term_growth_rate: float, periods: int):
    first_phase = np.sum(
        [(current_dividend * (1 + growth_rate) ** period) / (1 + discount_rate) ** period] for period in
        range(1, periods + 1))
    second_phase = gordon_growth_model(current_dividend=current_dividend * (1 + growth_rate) ** periods,
...
rate over a more gradual period (Gordon Growth Model). 
H is the half-life of the duration of that transitionary period
...
def dividend_discount_model_H_model(current_dividend: float, discount_rate: float, initial_growth_rate: float,
                                    terminal_growth_rate: float, periods: int):
    half_life = periods / 2
...
def dividend_discount_model_three_stage(current_dividend: float, discount_rate: float,
                                        initial_growth_rate: float, long_term_growth_rate: float,
                                        initial_period: int, transitionary_period: int):
    first_phase = np.sum(
        [(current_dividend * (1 + initial_growth_rate) ** period) / (1 + discount_rate) ** period] for period in
        range(1, initial_period + 1))  # inclusive to initial_period
    second_phase = dividend_discount_model_H_model(
        current_dividend=(current_dividend * (1 + initial_growth_rate) ** initial_period),  # dividend at initial_period
        discount_rate=discount_rate,
        initial_growth_rate=(1 + initial_growth_rate) ** initial_period,
        terminal_growth_rate=long_term_growth_rate,
        periods=transitionary_period)
    return first_phase + second_phase
        ...
        if model == 'DDM':
            ...
            if model == 'DDM H Model':
                ...
                return dividend_discount_model_three_stage(current_dividend=current_dividend,
                                                           discount_rate=discount_rate,
                                                           initial_growth_rate=growth_rate,
                                                           long_term_growth_rate=terminal_growth_rate,
                                                           initial_period=high_growth_periods,
                                                           transitionary_period=transitionary_growth_periods)
    return Exception
