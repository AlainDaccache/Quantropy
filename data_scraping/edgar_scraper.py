...
def normalization_second_level(input_dict, master_dict):
...
def normalize_tables(input_dict):
    ...
    master_dict = normalization_second_level(input_dict, master_dict)
...
def scrape_financial_statements(ticker, filing_type):
    ...
    for sheet_name, dict in zip(names,
        ...
        with open(financial_statements_file_path, "w") as csv:
            # keys = [str(k) for k, v in dict.items()]
            # keys.insert(0, 'Entries')
            # df = pd.DataFrame(dict, columns=keys)
            # df = df.reset_index().set_index('Entries')
